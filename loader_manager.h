...
public:
	...
	T getValue(const std::string& key, const T& defaultValue = T{})
	{
		...
		if (it != configValues.end() && std::holds_alternative<T>(it->second))
		{
...
private:
	...
	ConfigValue stringToValue(const std::string& str, const ConfigValue& defaultVal) const;
	static std::string getExecutableDirectory();
	void createDirectoryStructure() const;
...
void setupLoader();
LoaderManager* createLoaderManager(const std::string& name,

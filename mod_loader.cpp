...
std::string ModLoader::findFileOverride(const std::string& originalPath)
{
	...
	for (auto it = enabledMods.rbegin(); it != enabledMods.rend(); ++it)
	{
		...
		if (mod.allFiles.contains(normalizedPath))
		{
...
{
	{
		...
		if (preloadedCustomAssets.contains(normalizedPath))
		{
			return true;
		}
	}
...
void ModLoader::preloadCustomModAssets() const
{
			...
			if (!SnowdropIsFileAvailable(buffer, 0))
			{
				...
				if (SnowdropOpenFile)
				{
					...
					if (ctx && snowdropOpenFileHook((__int64)ctx, (_BYTE*)relativePath.c_str(), 1))
					{
						...
						using FuncType2 = __int64(__fastcall*)(__int64);
						auto sub_14B13CD50 = (FuncType2)0x14B13CD50;
				...
				{
					FRACTURE_ERROR(
						"SnowdropOpenFile function is not available, cannot preload custom assets, mods may not work correctly!");
				}
			}
		}
	}
	...
	for (size_t i = 0; i < loadedMods.size(); ++i)
	{
		...
		for (size_t j = i + 1; j < loadedMods.size(); ++j)
		{
			...
			for (const auto& file : loadedMods[i].allFiles)
			{
				...
				if (loadedMods[j].allFiles.contains(file))
				{
					loadedMods[i].hasConflict = true;
					loadedMods[j].hasConflict = true;
